...
# Run `docker build --file test-arm32v7-debian-mpich.dockerfile --build-arg date="$(date)" .`
...
RUN git clone -b eschnett/correct-mpi_f08 https://github.com/eschnett/MPItrampoline
...
RUN git clone -b eschnett/correct-mpi_f08 https://github.com/eschnett/MPIwrapper
